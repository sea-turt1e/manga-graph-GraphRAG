...
推薦作品を5作品選んで、それぞれについて以下の形式で回答してください。（作品タイトルの前にindexをつけてください）：
...
## 生成時の考慮点
推薦する際は以下の点を考慮してください：
### 1. グラフデータベース上の作品間の関係性（同じ作者、同じ雑誌、類似ジャンル）。
- 優先順位は次の通り（上が高優先度）。これらをバランスよく推薦すること。
- 理想は5作品のうち以下のような割合とする。（必須ではない）
  - (A) 2作品は同じ作者の作品
  - (B) 2作品は同じ雑誌に掲載された別の作者の作品
  - (C) 1作品は同出版社の他誌に掲載された別の作品


#### (A) 同じ作者の作品
例えば以下のようなcreatedの関係性（作者は同じ冨樫義博で、作品は別作品)
- 富樫義博 -[created]-> Hunter×hunter
- 富樫義博 -[created]-> 幽★遊★白書

#### (B) 同雑誌に掲載された別の作者の作品
例えば以下のようなcreatedとpublishedの関係性（雑誌は同じ週刊少年ジャンプ、作者は別の作者）
- 富樫義博 -[created]-> Hunter×hunter
- 週刊少年ジャンプ -[published]-> Hunter×hunter
- 猗笠怜司 -[created]-> 神のまにまに
- 週刊少年ジャンプ -[published]-> 神のまにまに

#### (C) 同出版社の他誌に掲載された別の作者の作品
例えば以下のようなpublishedとpublished_byの関係性（出版社は同じ集英社、雑誌は別のもの）
- 週刊少年ジャンプ -[published]-> Hunter×hunter
- 週刊少年ジャンプ -[published_by]-> 集英社
- 週刊ヤングジャンプ -[published]-> キングダム
- 週刊ヤングジャンプ -[published_by]-> 集英社

##### 実際のグラフデータベース上の関係性データ
{context}

### 2. ユーザーの好みとの関連性
なるべくジャンルの近いものを推薦してください。
### 3. なぜその作品を推薦するのか具体的な理由

## その他ルール
- 推薦作品を5作品選んで、それぞれについて以下の形式で回答してください。（作品タイトルの前にindexをつけてください）
- 英語タイトルなど同じ作品が複数ある場合は、正式な日本語タイトルを優先し、同じ作品は1つだけにしてください。
- 「ユーザーの好み」で指定された作品は含めないでください。
- レコメンド文のみを書いてください。（ユーザーの質問の分析結果などは生成しないでください）
- 「作者」や「雑誌」、「出版社」が「ユーザーの好み」と異なるものを推薦する場合は、「作者」や「雑誌」、「出版社」も理由に含めてください。
...
2. 複数候補がある場合は漫画の正式名のものを選択
3. 入力が曖昧な場合は推測して最も関連性が高い既存の有名漫画作品名1つを出力
4. 作品名が存在しない/不明な場合は生成しない。""（空）を出力
5. 英語で表記されている場合は英語のままにする
